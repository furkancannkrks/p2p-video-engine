...
// Görsel Enterpolasyon Döngüsü (P2P çöktükten sonra CDN sayacını takılmadan akıtır)
setInterval(() => {
...
if (Hls.isSupported() && p2pml.hlsjs.Engine.isSupported()) {
    ...
    // HLS Üzerinden Doğrudan CDN Sayacı (Fallback anında inen dev blokları yakalar)
    ...
    // 5. APPLE (iOS/Safari) KURTARMA VE SAYAÇ BLOĞU
    ...
    // Apple için UI dondurmayı engelleyen sayaç mantığı
    setInterval(() => {
        if (!video.paused) {
            if (video.webkitVideoDecodedByteCount) {
                totalCdn = video.webkitVideoDecodedByteCount; // Gerçek veriyi oku
            } else {
                totalCdn += 250000; // Safari veri vermezse UI akışı için tahmini yükleme
            }
            updateDashboardUI();
        }
    }, 1000);
